...
public class DvImageUtils {
	
	/** The tag. */
	private static String TAG = "DvImageUtil";
	
	/** The Constant D. */
	private static final boolean D = RunningModel.isDebugEnabled;
    ...
    */
	...
	public static byte[] view2Bytes(View view,Bitmap.CompressFormat compressFormat){  
		byte[] b = null;
		try {
			Bitmap bitmap = DvImageUtils.view2Bitmap(view);
			b = DvImageUtils.bitmap2Bytes(bitmap,compressFormat,true);
		} catch (Exception e) {
			e.printStackTrace();
		}  
	    return b;
	...
	*/
    ...
    public static String getHashCode(Bitmap bitmap){
        ...
        //第三步，计算平均值。
        //计算所有64个像素的灰度平均值。
        int avgPixel = DvMathUtils.average(pixels);
        ...
        for (int i = 0; i < comps.length; i+= 4) {
              ...
              hashCode.append(DvMathUtils.binaryToHex(result));
        }
